...
@section('content')
    ...
    <section class="relative bg-cover bg-center py-32"
        style="background-image: url({{ asset('static/images/hero-bg.jpg') }});">
        <div
            class="absolute inset-0 bg-gradient-to-b from-primary-700 to-primary-800 opacity-90 dark:from-primary-800 dark:to-primary-900 dark:opacity-90">
        </div>
                ...
                <div class="flex justify-center">
                    <form action="{!! route('names.search') !!}" method="GET" class="w-full max-w-lg">
                        ...
                        <div class="relative">
                            ...
                            <button type="submit"
                                class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-primary-500 text-surface px-4 py-3 rounded-full hover:bg-primary-600 transition duration-200">
    ...
    <section class="max-w-7xl mx-auto px-6 md:px-4 lg:px-8 my-10">
        ...
        <section class="container mx-auto py-20 px-10 mb-10 bg-surface dark:bg-base-800 shadow rounded-lg">
            ...
            <p class="text-center text-base-600 dark:text-base-300 mb-12">
                Explore the most popular names in our database. Find out the meaning, origin, and more.
            </p>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                @foreach ($popularNames->random(8) as $name)
                    <a href="{!! route('names.show', $name->slug) !!}"
                        class="group rounded-lg overflow-hidden shadow-md hover:shadow-lg border border-base-200 dark:border-none dark:bg-base-700 transition-all duration-300 ease-in-out">
                        <div class="p-8 text-center">
                            <h3
                                class="text-2xl font-bold text-base-900 dark:text-base-100 group-hover:text-primary-700 dark:group-hover:text-primary-500 transition-colors duration-300 truncate">
                                {{ $name->name }}</h3>
                            <p class="text-base-600 dark:text-base-300 mt-3 text-sm truncate capitalize">
                                {{ $name->meaning }}</p>
                            <span
                                class="text-primary-600 group-hover:text-primary-800 dark:text-primary-400 dark:group-hover:text-primary-200 text-sm font-semibold tracking-wide mt-4 inline-block transition-colors duration-300">
                    ...
                    <a href="{!! route('blog.show', $post->slug) !!}"
                        ...
                        <div class="p-6">
                            <h3
                                ...
                                {{ Str::limit($post->title, 25) }}</h3>
                            <p class="text-base-600 dark:text-base-300 text-sm mb-4">{{ Str::limit($post->excerpt, 100) }}
                            </p>
