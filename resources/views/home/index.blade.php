...
@section('content')
                ...
                <div class="flex justify-center">
                    <form action="{!! route('names.search') !!}" method="GET" class="w-full max-w-lg relative">
                        ...
                        <input type="text" id="search-home" name="q"
                               class="w-full pl-5 pr-16 py-3 rounded-full bg-white dark:bg-slate-800 bg-opacity-90 text-slate-700 dark:text-slate-300 focus:bg-opacity-100 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:outline-none transition duration-200"
    ...
    <section class="container mx-auto px-4 sm:px-6 lg:px-12 py-20 dark:bg-slate-800 my-10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
            @foreach ($data['popularNames'] as $name)
                <a href="{!! route('names.show', $name->slug) !!}" class="group bg-white dark:bg-slate-800 rounded-lg overflow-hidden shadow-md hover:shadow-xl border border-slate-200 dark:border-slate-700 transform hover:scale-105 transition-all duration-300 ease-in-out">
    ...
    <section class="py-20 dark:bg-slate-800 my-10">
        <div class="container mx-auto">
            <h2 class="text-5xl font-extrabold text-center text-slate-800 dark:text-slate-100 mb-16">Insights & Stories</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                @foreach ($data['latestPosts'] as $post)
                    <a href="{!! route('blog.show', $post->slug) !!}" class="group bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden shadow transition-transform duration-300 hover:shadow-lg">
{{--                        <img class="w-full h-56 object-cover group-hover:opacity-90 transition-opacity duration-300" src="{{ $post->featured_image }}" alt="{{ $post->title }}">--}}
                        <div class="p-6">
                            <h3 class="text-2xl font-semibold text-slate-800 dark:text-slate-100 group-hover:text-indigo-600 mb-2 transition-colors duration-300">{{ Str::limit($post->title, 25) }}</h3>
                            <p class="text-slate-600 dark:text-slate-300 text-sm mb-4">{{ Str::limit($post->excerpt, 100) }}</p>
                            <div class="text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wide font-semibold">
