<section class="text-base-900 dark:text-base-100 px-6 py-8 shadow rounded-lg border dark:border-base-700 bg-surface dark:bg-base-800">
    ...
    <a download id="download-link" class="group text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-200 font-semibold transition-colors duration-300 flex items-center justify-end mt-4" href="{!! $data['wallpapers']->first() !!}" target="_blank">
        Download
    ...
    <div class="overflow-hidden rounded-lg shadow-lg dark:shadow-none my-8">
        <img
            ...
            data-src="{!! $data['wallpapers']->first() !!}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 justify-center">
        @foreach ($data['wallpapers']->random(2) as $url)
        <img
