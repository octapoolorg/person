...
@section('content')
    ...
    <section class="max-w-7xl mx-auto px-6 md:px-4 lg:px-8">
        ...
        <!-- Blog Header Section -->
        <section class="bg-indigo-600 text-white py-10 bg-gradient-to-r from-blue-500 to-indigo-600 mt-8 md:mt-20">
        ...
        <section class="container py-12">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                @foreach ($posts as $post)
                    <a href="{!! route('blog.show', $post->slug) !!}" class="rounded-lg shadow overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        ...
                        @endif
                        <div class="p-6">
                            <h2 class="text-2xl font-semibold mb-2 text-slate-900">{{ $post->title }}</h2>
                            <p class="text-slate-600 mb-4">{{ Str::limit($post->excerpt, 150) }}</p>
